...
const categories = ["Wszystkie", "E-Commerce", "Marketing", "DevOps"];
          ...
          {filteredProjects.map((project) => (
            ...
            >
              <Card className="h-full bg-card/40 backdrop-blur-sm border-border/50 hover:border-primary/50 hover:shadow-lg transition-all duration-300 group overflow-hidden flex flex-col">
                <div className="h-48 w-full bg-black/60 border-b border-border/50 relative flex items-center justify-center">
                   <span className="text-muted-foreground/50 font-mono text-sm group-hover:scale-110 transition-transform duration-500">
                     [ Miniatura: {project.title} ]
                   </span>
                </div>
        ...
        {selectedProject && (
          ...
          >
            <motion.div
              ...
              // UWAGA: Główne okno jest "overflow-hidden", więc przycisk (X) nie będzie uciekał
            ...
            >
              {/* Przycisk (X) zawsze w tym samym miejscu, odporny na scroll */}
              <button
                onClick={() => setSelectedProject(null)}
              ...
              {/* Zagnieżdżony scrollowalny pojemnik - to on przyjmuje suwak */}
              <div className="w-full h-full max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none][scrollbar-width:none]">
                ...
                <div className="w-full h-64 md:h-96 bg-[#05080f] relative flex items-center justify-center border-b border-[#1F2937]">
                   <span className="text-muted-foreground font-mono">[ Główne Zdjęcie: {selectedProject.mainImage} ]</span>
                </div>
                ...
                <div className="p-6 md:p-10 flex flex-col gap-8">
                  <div>
                    ...
                    <p className="text-muted-foreground text-base md:text-lg leading-relaxed">
                      ...
                      <br/><br/>
                      Tutaj możesz dodać dłuższy opis, opowiedzieć z jakimi problemami się zmierzyłeś podczas kodowania i w jaki sposób udało Ci się je rozwiązać.
                    </p>
                  </div>

                  {/* Sekcja Galerii w poziomie z customowym paskiem przewijania */}
                  <div>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 snap-x [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-[#111827][&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/30 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-primary/60 transition-colors">
                      {selectedProject.gallery.map((img, i) => (
                        <div key={i} className="min-w-60 md:min-w-75 h-40 bg-[#111827] border border-[#1F2937] rounded-xl flex items-center justify-center shrink-0 snap-center">
                          <span className="text-xs text-muted-foreground font-mono">[ Miniatura z galerii ]</span>
                        </div>
                      ))}
                    </div>
                  </div>
